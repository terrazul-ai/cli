...
describe('integration: publish command', () => {
  ...
  beforeEach(async () => {
    ...
    // Create minimal publishable package
    await fs.writeFile(
  ...
  it('publishes to dummy registry', async () => {
    const env = { ...process.env, HOME: tmpHome, USERPROFILE: tmpHome };
    const { stdout } = await run('node', [cli, 'publish'], { cwd: tmpPkg, env });
    // Dummy registry returns version 1.0.0 in response; accept any semver
    expect(stdout).toMatch(/Published @e2e\/publish-demo@\d+\.\d+\.\d+/);
  });
});
