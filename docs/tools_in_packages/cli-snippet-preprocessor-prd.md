...
- Porting tool invocation, tool output formatting, and config helper logic from the source repository (`terrazul/cli`).
- Implementing the new snippet-driven renderer inside the target CLI project (`cli`).
...
| Action                       | File(s)                                                                                                                                                                                                        | Notes                                                                                                                                                                                                         |
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Port (copy verbatim)**     | `cli/src/utils/tool-runner.ts`<br>`cli/src/utils/tool-output.ts` _(source repo: `terrazul/cli/src/utils/{tool-runner,tool-output}.ts` → target repo: `cli/src/utils/{tool-runner,tool-output}.ts`)_            | Keep implementations unchanged to retain established safe-mode behavior and output formatting.                                                                                                                |
| **Port (selective extract)** | `cli/src/utils/config.ts` _(source repo: `terrazul/cli/src/utils/config.ts`)_                                                                                                                                  | Bring over `ToolSpec` definitions, defaults (`DEFAULT_COMMANDS`, `DEFAULT_PROFILE_TOOLS`), `normalizeToolSpec()`, `computeOutputTargets()`, `selectPrimaryTool()`, and env helpers; omit YAML-specific logic. |
| **Extract & Rehost**         | `cli/src/core/task-runner.ts` → new `cli/src/core/destinations.ts` _(source repo: `terrazul/cli/src/core/task-runner.ts`)_                                                                                     | Move `resolveWritePath`, `ensureFileDestination`, `safeResolveWithin`, `DIRECTORY_DEFAULT_FILENAMES`, and related helpers to support safe output writes in `cli/src/core/destinations.ts`.                    |
| **Refactor**                 | `cli/src/commands/run.ts`, `cli/src/commands/apply.ts` \*(source repo) → `cli/src/commands/...`                                                                                                                | Preserve CLI option handling and manifest lookup; replace YAML task invocation with calls into the snippet renderer + destination utilities.                                                                  |
